...
export default function KayitEkrani() {
  const { kayitOlEposta, googleIleGiris } = useAuth();
  const { t } = useGorunum();
  ...
  async function kayitOlTikla() {
    setHata(null);
    if (!adSoyad.trim() || !eposta.trim() || !sifre) {
      setHata(t("lutfenDoldurun"));
      return;
    }
    if (sifre.length < 6) {
      setHata(t("sifreBoyut"));
      return;
    }
    setKayitYukleniyor(true);
    const sonuc = await kayitOlEposta(eposta, sifre, adSoyad);
    ...
    if (sonuc.epostaDogrulamaGerekli) {
      Alert.alert(
        t("epostaDogrulaBaslik"),
        t("epostaDogrulaMesaj"),
        [{ text: t("tamam"), onPress: () => router.replace("/(auth)/giris") }]
      );
    }
  }
      ...
      >
        <View style={styles.dis}>
          ...
          <View style={styles.ust}>
            ...
            <Text style={styles.altBaslik}>{t("kayitBaslik")}</Text>
          </View>
          ...
          <View style={styles.kart}>
            ...
            <TouchableOpacity
              ...
              onPress={googleTikla}
              disabled={googleYukleniyor || kayitYukleniyor}
              activeOpacity={0.75}
            >
              {googleYukleniyor ? (
                ...
                <>
                  <GoogleG />
                  <Text style={styles.googleMetin}>{t("googleKayit")}</Text>
                </>
              )}
            </TouchableOpacity>
            ...
            <View style={styles.inputGrup}>
              <Text style={styles.inputLabel}>{t("nasılHitap")}</Text>
              <TextInput
                style={styles.input}
                placeholder={t("isimPh")}
            ...
            <TouchableOpacity
              style={[styles.ileriButon, kayitYukleniyor && styles.pasif]}
              onPress={kayitOlTikla}
              disabled={kayitYukleniyor || googleYukleniyor}
              activeOpacity={0.85}
            >
              {kayitYukleniyor ? (
                <ActivityIndicator size="small" color="#fff" />
                ...
                <>
                  <Text style={styles.ileriMetin}>{t("kayitOlBtn")}</Text>
            ...
            <View style={styles.altSatir}>
              <Text style={styles.altMetin}>{t("hesapVarMi")}</Text>
              <Link href="/(auth)/giris" asChild>
                <TouchableOpacity>
                  <Text style={styles.kayitLink}>{t("girisYapLink")}</Text>
                </TouchableOpacity>
              </Link>
            </View>
